    ...
    <div className="space-y-10">
      ...
      <div className="grid gap-8">
        {visibleTours.map((tour) => {
          const isOpen = selectedTour === tour.slug;

          return (
            <article
              id={tour.slug}
              key={tour.slug}
              className="card overflow-hidden border-forest-900/10"
            >
              <div className="grid gap-0 lg:grid-cols-[0.95fr_1.05fr]">
                ...
                <div className="p-7 md:p-9">
                  ...
                  <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                      <h3 className="font-heading text-3xl text-forest-900 md:text-4xl">
                        {tour.title}
                      </h3>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    {tour.highlights.map((highlight) => (
                      <span
                        key={highlight}
                        className="rounded-full border border-forest-900/10 px-4 py-2 text-sm text-charcoal-600"
                      >
                        {highlight}
                      </span>
                    ))}
                  ...
                  <div className="mt-8 flex flex-col gap-4 sm:flex-row">
                    <button
                      type="button"
                      onClick={() => setSelectedTour(isOpen ? null : tour.slug)}
                      className="btn-primary"
                    >
                      {isOpen ? "Hide Itinerary" : "View Full Itinerary"}
                    </button>
                  ...
                  {isOpen ? (
                    <div className="mt-8 rounded-[1.5rem] bg-sand-50 p-6">
                      <h4 className="font-heading text-2xl text-forest-900">
                        Day-by-day itinerary
                      </h4>
                      <div className="mt-5 space-y-4">
                        {tour.itinerary.map((day) => (
                          <p key={day} className="text-base leading-8 text-charcoal-700">
                            {day}
                          </p>
                        ))}
                      </div>
